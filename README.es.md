...
* **Suite de tests (504 tests):** 430 tests unitarios que cubren la lógica de negocio sin Spring context ni base de datos, más 74 tests de integración con **Testcontainers** (PostgreSQL real en Docker) que cubren autenticación, reglas de ownership, estadísticas, pagos, producción y comisiones, el cierre de citas por rol, los módulos activables del negocio y el flujo completo del webhook de Stripe con verificación de firma real.
...
**504 tests** se ejecutan en CI en cada push (GitHub Actions).
...
* **ModuloIntegrationTest** (10) — los módulos de punta a punta, que es donde se ve lo que un permiso no puede hacer: con el cobro apagado **un ADMIN recibe 409** (no 403: no es «tú no puedes», es que aquí no se cobra), apagar solo la tarjeta deja en pie el cobro en el local, y volver a encenderlo devuelve el cobro sin reiniciar. Además: `/api/modulos/activos` se lee **sin token** porque la app tiene pantallas públicas, el catálogo y su escritura son solo del ADMIN, apagar los tres medios de pago apaga el cobro entero, y —lo que este diseño tenía que evitar— **con los pagos apagados la producción cuenta lo realizado** en vez de quedarse a cero, sin tocar el porcentaje ya congelado en las citas cerradas.
