...
* **Validación de horario laboral:** las citas solo se pueden agendar de lunes a sábado de 9:00 a 20:00, y nunca en el pasado. El horario y los días de la semana cerrados viven en la tabla `negocio` y se editan desde el panel, así que cada peluquería fija el suyo sin tocar el despliegue.
...
* **Permisos por rol configurables:** un administrador afina desde el panel qué puede hacer cada rol —hoy, que un peluquero cobre en efectivo sus propias citas (`PAGO_MANUAL_REGISTRAR`), que mueva de fecha las de su agenda (`CITA_REPROGRAMAR`) qué puede hacer en la galería (`GALERIA_SUBIR`, `GALERIA_EDITAR_PROPIA`, `GALERIA_EDITAR_AJENA` y `GALERIA_ORDENAR`) y que rellene su propio CV público (`PERFIL_CV_EDITAR`)—. La regla que lo sujeta todo es que **un permiso estrecha y nunca abre**: se consultan desde los services, después de la regla de rol de `SecurityConfig`, así que encender uno no concede nada que el rol ya prohibiera. El catálogo es un enum del código y la base de datos solo guarda el estado, con la ausencia de fila valiendo el valor por defecto: desplegar un permiso nuevo no cambia lo que puede hacer nadie hasta que se enciende a mano.
...
* **Instalación por peluquería, y un script que la da de alta:** los datos del negocio (nombre, contacto, marca y horario) viven en la base y se editan desde el panel, no en el despliegue ni en el código de la app. `scripts/alta-cliente.sh` deja lista una instalación recién levantada —primera cuenta de administrador, ficha del negocio y perfil de módulos— y `docs/alta-cliente.md` tiene el orden completo, incluido lo que no se puede automatizar.
* **Suite de tests (542 tests):** 452 tests unitarios que cubren la lógica de negocio sin Spring context ni base de datos, más 90 tests de integración con **Testcontainers** (PostgreSQL real en Docker) que cubren autenticación, reglas de ownership, estadísticas, pagos, producción y comisiones, el cierre de citas por rol, los módulos activables del negocio y el flujo completo del webhook de Stripe con verificación de firma real.
...
**542 tests** se ejecutan en CI en cada push (GitHub Actions).
...
| PagoServiceTest | 47 | PaymentIntents, webhooks, pago manual, reembolsos, polling, concurrencia, quién puede pedir un recibo y en qué estados, y quién puede cobrar en efectivo: un peluquero necesita el permiso **y** que la cita sea de su agenda, mientras que un ADMIN no pasa por los permisos. Los **módulos** se prueban aquí por lo contrario: con el cobro apagado no cobra **ni un ADMIN**, cada medio de pago mira el suyo, y el webhook y el reembolso siguen funcionando, porque apagar un módulo no puede dejar dinero en el aire en Stripe |
...
| GaleriaServiceTest | 35 | Galería: sube imagen y miniatura al bucket público, la clave la genera el servidor y separa tamaños, sin miniatura la respuesta cae a la grande, rechaza lo que no es imagen (también la miniatura), la foto nueva se coloca detrás de la última, borrar quita **los dos** objetos, y el **dueño de cada foto**: la subida se sella con la cuenta autenticada, cada peluquero edita y borra las suyas y no las de un compañero, una foto sin dueño es del negocio y cuenta como ajena, reordenar es un permiso aparte porque mueve la rejilla de todos, y un ADMIN no pasa por los permisos. Del **módulo**: apagado no hay galería **ni para un ADMIN**, y tampoco se lista sin cuenta —devolver una lista vacía diría «no hay fotos», que es otra cosa— |
...
| ModuloServiceTest | 23 | Los módulos del negocio, que son lo contrario de un permiso: **nacen encendidos** (desplegarlos no le quita nada a nadie), apagar el padre apaga los hijos, apagar todos los hijos apaga el padre de hecho (si no quedaría «encendido» un cobro sin ninguna forma de cobrar), un hijo encendido bajo un padre apagado se marca como no efectivo para que la pantalla no mienta, el 409 nombra al padre cuando es el padre quien está apagado, una fila de un módulo retirado del código se ignora, y escribir tira la caché. Y los **perfiles de arranque**, que escriben el estado de todos los módulos de una vez para dar de alta una peluquería nueva: aplicarlos dos veces deja lo mismo, tiran la caché, y ni «solo agenda» ni «agenda y caja» encienden la pasarela de tarjeta, porque Stripe necesita una cuenta propia de cada negocio |
| NegocioServiceTest | 11 | La ficha del negocio, que es lo que hace falta para instalarle esto a otra peluquería: el horario sale de la tabla y ya no de `application.properties` (mientras estuvo ahí no podía ser distinto para cada cliente sin tocar el despliegue y reiniciar), se lee una sola vez (caché) y guardar la tira, una apertura posterior o igual al cierre se rechaza con un mensaje en vez de dejar saltar el CHECK de la tabla como un 500, los campos opcionales vacíos se guardan como nulos, un día de la semana que no existe se ignora en vez de tumbar el arranque, y sin fila se dice que la instalación está incompleta en lugar de inventarse un nombre y un horario |
...
| PeluqueroCvServiceTest | 16 | El CV público: el listado sale del repositorio que **filtra los inactivos y ordena** (no del `findByActivoTrue` de siempre, que no garantiza ningún orden), una ficha sin foto no inventa una URL, una cuenta sin ficha no tiene CV, sin `PERFIL_CV_EDITAR` no lo rellena, un ADMIN no pasa por la matriz, un campo en blanco **borra** en vez de guardar espacios, del Instagram se guarda solo el usuario aunque se pegue la URL entera, un peluquero no toca la foto de un compañero ni con el permiso encendido, y reemplazar la foto **guarda antes de borrar** la anterior. Del **módulo**: apagado no se publican fichas ni se edita ningún CV, ni el propio ni el de otro |
| EspecialidadesTest | 8 | La única pieza que sujeta guardar las especialidades como texto con comas: trocear y limpiar en los dos sentidos, quitar repetidas ignorando mayúsculas, rechazar una etiqueta **con una coma dentro** (partiría la lista al releerla) y rechazar lo que no cabe en la columna aunque cada etiqueta por separado sea válida |
| PeluqueroServiceTest | 25 | CRUD y soft delete, vínculo con la cuenta (rechaza un `USER`, una cuenta desactivada y una ya vinculada a otra ficha), desvincular, y la comisión aplicable: la excepción por servicio gana al porcentaje de la ficha. Con el **módulo de comisiones apagado**, su endpoint responde 409, la ficha sale sin porcentaje y sin excepciones —el dato sigue guardado— y se sigue pudiendo editar el nombre: lo que desaparece es el dinero, no la ficha |
| ProduccionServiceTest | 15 | Mapeo del resumen, los desgloses y lo pendiente de cobro, cuenta sin ficha vinculada, rango invertido y de más de dos años, y el redondeo a dos decimales de lo que devuelve Postgres. Y los dos módulos: con **los pagos apagados** cuenta lo realizado aunque no esté cobrado (si no, toda la producción sería cero) y desaparece el «sin cobrar», que si no contaría dos veces el mismo trabajo; con **las comisiones apagadas** la comisión no viaja ni en el total, ni en las líneas, ni en la comparativa |
...
| RecordatorioCitaSchedulerTest | 6 | Recordatorio 24h: envío único, ignora anuladas/ya notificadas, Clock inyectable, y con el módulo apagado no manda nada **y no marca ninguna cita como avisada**, que es lo que dejaría sin aviso a toda esa ventana al volver a encenderlo |
...
### Tests de integración (90, Testcontainers)
...
* **GaleriaIntegrationTest** (4) — la asimetría de la galería por HTTP: el listado se lee **sin token**, y subir, ordenar y borrar responden 403 sin token y con un cliente. Otro comprueba lo que no se puede ver en un test unitario del service: que una foto subida sin miniatura devuelve la grande en su lugar, y que un ejecutable renombrado a `.jpg` se queda en 400 tras pasar por el multipart de verdad. Los dos últimos cruzan **dos peluqueros y sus fotos**: cada uno maneja las suyas y recibe 403 en las del compañero, el listado marca cuáles son suyas sin publicar su id ni su email, reordenar alcanza a cualquier foto porque renumera la rejilla entera, y las fotos sin dueño (las del negocio) solo las toca un ADMIN.
* **PeluqueroCvIntegrationTest** (7) — el CV es el segundo recurso que se lee **sin cuenta**, y el primero que sale de una tabla que además guarda la cuenta vinculada y lo que cobra cada uno; por eso se comprueba por HTTP que el JSON de `/api/peluqueros/publicos` **no lleva email, teléfono, `usuarioId`, comisión ni `activo`**, y que su regla está de verdad antes de la de `/api/peluqueros/**` en la cadena de filtros. Además: una ficha sin CV se presenta solo con el nombre y con una lista vacía (no `null`, que rompería el `*ngFor`), solo salen los activos y en el orden que puso el ADMIN con desempate por nombre, ver su propia ficha no pide permiso pero escribirla sí, `PUT /{id}/cv` es de ADMIN incluso sobre su propia ficha (para eso está `/mio`), vaciar un campo es mandarlo en blanco, una especialidad con coma y unas que no caben en la columna se quedan en 400 y no en un 500 de Postgres, y la foto la pone el dueño de la ficha o el ADMIN, nunca un compañero.
...
* **ModuloIntegrationTest** (17) — los módulos de punta a punta, que es donde se ve lo que un permiso no puede hacer: con el cobro apagado **un ADMIN recibe 409** (no 403: no es «tú no puedes», es que aquí no se cobra), apagar solo la tarjeta deja en pie el cobro en el local, y volver a encenderlo devuelve el cobro sin reiniciar. Además: `/api/modulos/activos` se lee **sin token** porque la app tiene pantallas públicas, el catálogo y su escritura son solo del ADMIN, apagar los tres medios de pago apaga el cobro entero, y —lo que este diseño tenía que evitar— **con los pagos apagados la producción cuenta lo realizado** en vez de quedarse a cero, sin tocar el porcentaje ya congelado en las citas cerradas. Y los cuatro módulos que **quitan una parte entera del producto**: apagar la galería o el CV del equipo se lleva su listado **también sin token** (un 409 dice «aquí no hay galería», que no es lo mismo que una lista vacía), elegir con quién agendar sigue funcionando porque eso no sale del CV, y apagar producción responde 409 **también a un ADMIN**. Y los **perfiles de arranque**: aplicar uno apaga todo lo que no es suyo de una vez, el de caja no enciende la pasarela de tarjeta, solo lo aplica un ADMIN y un perfil que no existe es 400 y no 500.
* **NegocioIntegrationTest** (7) — la ficha del negocio de punta a punta, que es donde se ven las dos cosas que justifican esta tabla: la fila **la siembra la migración** (una instalación recién levantada ya tiene horario y se puede agendar sin que nadie rellene nada; si no estuviera, el primer `/api/citas` sería un 500), y **cambiar el horario desde el panel mueve los huecos que se ofrecen al agendar**, que es lo que antes obligaba a editar el despliegue y reiniciar. Además: la ficha se lee **sin token** porque las dos apps la pintan antes del login, editarla es solo del ADMIN, un horario al revés responde 400 y no un 500 del CHECK de la tabla, el nombre es obligatorio y el resto no (una peluquería recién dada de alta puede no tener aún ni teléfono), y lo guardado se ve en la siguiente lectura sin reiniciar.
* **PermisoIntegrationTest** (11) — los permisos configurables de punta a punta, que es donde se ve el reparto entre las dos capas: `SecurityConfig` deja llegar al service a quien *podría* tener el permiso y el service decide si de verdad lo tiene. Con el permiso apagado un peluquero recibe 403 al cobrar y el ADMIN cobra igual; encendido cobra la suya pero **no la de un compañero**; a un cliente no le sirve de nada que esté encendido, porque le para la regla de rol antes; apagarlo se lo quita otra vez (la caché no lo vuelve irrevocable); cada uno consulta los suyos y solo el ADMIN ve la matriz, que no ofrece casillas para ADMIN ni para cliente; y escribir una clave inexistente o un rol que no se configura responde 400 en vez de dejar una fila que nadie leería. Y el catálogo, que no es un permiso: un peluquero con la matriz entera apagada lee `GET /api/servicios` con su token, porque la app le da una pestaña con él, y un servicio dado de baja por el ADMIN ya no le aparece.
...
| POST | `/api/galeria` | PELUQUERO, ADMIN | Subir una foto al final de la rejilla (`multipart/form-data`: `imagen`, `miniatura` opcional, `titulo` opcional). Queda sellada con la cuenta que la sube |
| PUT | `/api/galeria/{id}` | Dueño, ADMIN | Cambiar el título o la posición |
| DELETE | `/api/galeria/{id}` | Dueño, ADMIN | Borrar la foto y sus dos objetos del almacén |

> Un ADMIN toca cualquier foto. Un PELUQUERO depende de sus permisos: `GALERIA_SUBIR` para publicar, `GALERIA_EDITAR_PROPIA` para el título y el borrado de **las suyas**, `GALERIA_EDITAR_AJENA` para las de otro y `GALERIA_ORDENAR` para mover la rejilla, que va aparte porque colocar una foto renumera las de todos. Las fotos sin dueño son «del negocio» y cuentan como ajenas.
...
| Método | Endpoint | Acceso | Descripción |
|--------|----------|--------|-------------|
| GET | `/api/peluqueros/publicos` | **Público** | El equipo con su CV, para elegir antes de registrarse. Solo activos y ordenados; no publica nada de la cuenta ni la comisión |
...
| PUT | `/api/peluqueros/{id}` | ADMIN | Actualizar peluquero: `nombre`, `activo`, `comisionPorcentaje`, `orden`, `usuarioId` para vincular la cuenta o `desvincularUsuario` |
| GET | `/api/peluqueros/mio` | PELUQUERO, ADMIN | Su propio CV, resuelto desde la cuenta (sin id, como `/produccion/mia`) |
| PUT | `/api/peluqueros/mio` | PELUQUERO con `PERFIL_CV_EDITAR`, ADMIN | Reemplaza su CV entero: lo que no se manda se borra |
| PUT | `/api/peluqueros/{id}/cv` | ADMIN | El CV de cualquier ficha. Ningún permiso lo abre a un peluquero |
| POST | `/api/peluqueros/{id}/foto` | Dueño de la ficha con `PERFIL_CV_EDITAR`, ADMIN | Foto del CV (multipart `foto`, validada por magic bytes) |
| DELETE | `/api/peluqueros/{id}/foto` | Dueño de la ficha con `PERFIL_CV_EDITAR`, ADMIN | Quitar la foto del CV (idempotente) |
...
>
> Salvo que el negocio **no cobre por la aplicación**: con el módulo `PAGOS` apagado nada llegaría nunca a `PAGADO`, así que ahí se cuenta lo `COMPLETADA` a secas y la respuesta lo dice en `exigeCobro`. Con `COMISIONES` apagado la comisión viaja como `null`, que no es cero: cero sería «trabaja al 0 %».
...
| Método | Endpoint | Acceso | Descripción |
|--------|----------|--------|-------------|
| GET | `/api/modulos/activos` | **Público** | Claves de lo que este negocio tiene encendido. Sin token porque la app tiene pantallas que se ven sin cuenta y necesita saber qué pintar antes del login |
| GET | `/api/modulos` | ADMIN | Catálogo completo con su estado, para la pantalla de configuración |
| PUT | `/api/modulos` | ADMIN | Encender y apagar (`cambios[]` con `clave` y `activo`) |
| GET | `/api/modulos/perfiles` | ADMIN | Los perfiles de arranque, con lo que enciende y lo que apaga cada uno |
| POST | `/api/modulos/perfiles/{clave}` | ADMIN | Aplicar un perfil: escribe el estado de **todos** los módulos de una vez |

> **Un módulo no es un permiso, y confundirlos rompe el diseño.** Un permiso responde a «quién puede hacer X» y nunca alcanza al ADMIN; un módulo responde a «este negocio hace X en absoluto» y apagado **también desaparece para el administrador**. Por eso responde **409** y no 403: no es «tú no puedes», es que aquí eso no existe; y no 404, que haría pensar que el backend desplegado es viejo. El orden al comprobar es siempre **módulo → rol → permiso**.
>
> Igual que los permisos, el catálogo lo define el enum `Modulo` y la tabla solo guarda el estado. La diferencia es el valor por defecto: **todos nacen encendidos**, porque el defecto tiene que ser el comportamiento de antes. Los medios de pago cuelgan de `PAGOS`: apagar el padre los apaga, y apagarlos todos apaga el padre, para que no quede un cobro «encendido» sin ninguna forma de cobrar.
>
> Hoy el catálogo son nueve: `COMISIONES`, `PAGOS` con sus tres medios de pago, y los cuatro que quitan una parte entera del producto —`GALERIA`, `EQUIPO_CV`, `PRODUCCION` y `RECORDATORIOS_EMAIL`—. El **asistente no está y no debe estar**: se apaga por configuración del despliegue porque consume cuota de una API de pago, así que es coste de infraestructura y no una decisión de negocio; la pantalla lo dice para que la pregunta no vuelva cada vez.
>
> Apagar los recordatorios **no marca ninguna cita como avisada**: si lo hiciera, volver a encenderlos dejaría sin aviso a todas las citas de esa ventana. El scheduler sigue corriendo y simplemente no manda nada. Y no alcanza a los demás correos: el de una cita agendada o anulada confirma algo que acaba de pasar, no es un recordatorio.
>
> Apagar un módulo **no borra datos ni reescribe el histórico**: los porcentajes guardados se quedan, una cita cerrada conserva su comisión congelada, y un `PaymentIntent` vivo se puede seguir cobrando o cancelando. Lo que se apaga es lo que se ofrece y se calcula de aquí en adelante.
>
> **Los perfiles de arranque** existen porque «todos encendidos» es lo correcto para un negocio que ya venía funcionando y justo lo contrario de lo que quiere uno que acaba de entrar: a ese se le enseña de golpe el producto entero y hay que ir apagándole nueve cosas a mano. Hay tres —`SOLO_AGENDA`, `AGENDA_Y_CAJA` y `TODO`— y **un perfil describe el estado completo**, no una suma sobre lo que hubiera antes: todo lo que no esté en él se apaga, así que aplicarlo dos veces deja lo mismo. No se guarda en ninguna columna cuál se aplicó, porque sería un dato que empieza a mentir en cuanto alguien toque una casilla. Y ni `SOLO_AGENDA` ni `AGENDA_Y_CAJA` encienden `PAGO_TARJETA`: Stripe necesita una cuenta y unas claves propias de cada negocio, así que el primer día de un cliente nuevo encenderlo sería ofrecerle al cliente final una pasarela que devuelve error.

### El negocio
| Método | Endpoint | Acceso | Descripción |
|--------|----------|--------|-------------|
| GET | `/api/negocio` | **Público** | Nombre, contacto, marca y horario. Sin token porque las dos apps lo pintan antes del login |
| PUT | `/api/negocio` | ADMIN | Guardar la ficha entera |

> **Es lo que hace falta para instalarle esto a otra peluquería.** El horario vivía en `application.properties` (`peluqueria.horario.*`) y el nombre, el teléfono y la dirección estaban escritos en el código del móvil: cambiarlos era editar el despliegue y reiniciar, o recompilar y reinstalar la APK. Desde la `V17` es una fila de la tabla `negocio` y un formulario del panel.
>
> **Una sola fila, y se fuerza con un CHECK** en vez de confiar en que nadie inserte otra. Este backend sirve a una peluquería: el día que sirva a varias, esta es la tabla que lleva el `negocio_id` y el CHECK se cae. Que la fila única sea explícita es lo que hace que ese día se note.
>
> El **horario de aquí es el fijo semanal** (a qué hora abre y qué días no abre nunca). Los festivos y los cierres de un día concreto siguen siendo `dias_bloqueados`, que van por fecha y son otra pregunta. Lo que **no** está aquí y es de la instalación: el remitente del correo (`MAIL_FROM`, que lo impone el proveedor) y las claves de Stripe.
>
> Se cachea en memoria por lo mismo que los módulos y los permisos —el horario se pregunta en el camino de cada petición que agenda o pinta huecos— y guardar tira la caché, para que cambiarlo mueva los huecos ofrecidos sin reiniciar.
...
3. **Configurar variables de entorno** (en tu sistema o IDE):
    ...
    *(El horario laboral no es una property: vive en la tabla `negocio`, la siembra la migración y se edita desde el panel. Una instalación nueva arranca con 09:00-20:00 y el domingo cerrado.)*
