...
* **Suite de tests (514 tests):** 437 tests unitarios que cubren la lógica de negocio sin Spring context ni base de datos, más 77 tests de integración con **Testcontainers** (PostgreSQL real en Docker) que cubren autenticación, reglas de ownership, estadísticas, pagos, producción y comisiones, el cierre de citas por rol, los módulos activables del negocio y el flujo completo del webhook de Stripe con verificación de firma real.
...
**514 tests** se ejecutan en CI en cada push (GitHub Actions).
...
| ModuloServiceTest | 15 | Los módulos del negocio, que son lo contrario de un permiso: **nacen encendidos** (desplegarlos no le quita nada a nadie), apagar el padre apaga los hijos, apagar todos los hijos apaga el padre de hecho (si no quedaría «encendido» un cobro sin ninguna forma de cobrar), un hijo encendido bajo un padre apagado se marca como no efectivo para que la pantalla no mienta, el 409 nombra al padre cuando es el padre quien está apagado, una fila de un módulo retirado del código se ignora, y escribir tira la caché |
...
### Tests de integración (64, Testcontainers)
...
* **ModuloIntegrationTest** (13) — los módulos de punta a punta, que es donde se ve lo que un permiso no puede hacer: con el cobro apagado **un ADMIN recibe 409** (no 403: no es «tú no puedes», es que aquí no se cobra), apagar solo la tarjeta deja en pie el cobro en el local, y volver a encenderlo devuelve el cobro sin reiniciar. Además: `/api/modulos/activos` se lee **sin token** porque la app tiene pantallas públicas, el catálogo y su escritura son solo del ADMIN, apagar los tres medios de pago apaga el cobro entero, y —lo que este diseño tenía que evitar— **con los pagos apagados la producción cuenta lo realizado** en vez de quedarse a cero, sin tocar el porcentaje ya congelado en las citas cerradas. Y los cuatro módulos que **quitan una parte entera del producto**: apagar la galería o el CV del equipo se lleva su listado **también sin token** (un 409 dice «aquí no hay galería», que no es lo mismo que una lista vacía), elegir con quién agendar sigue funcionando porque eso no sale del CV, y apagar producción responde 409 **también a un ADMIN**.
