...
* **Business-hours validation:** appointments can only be booked Monday to Saturday, 9:00-20:00, never in the past. Opening hours and closed weekdays live in the `negocio` table and are edited from the admin panel, so each salon sets its own without touching the deployment.
...
* **Work gallery:** photos of the salon's work with **manual ordering**, in a public bucket (`galeria`) because they are promotional material. They are read **without an account**, like the catalog. Each photo records **who uploaded it**: a hairdresser edits and deletes their own — and only if their role has the permission enabled — while an ADMIN touches any of them; ownerless photos belong to the salon. Each photo stores **two keys**, the image and a thumbnail: the grid always renders the thumbnail and the full-size image is fetched only when a photo is opened, because the free Storage plan's limit is **bandwidth** and a grid served with full-size images multiplies it by ten. The thumbnail is generated client-side and travels in the same multipart — the server has 0.1 CPU in production and the browser scales for free — is validated by magic bytes just like the big one and, if it is missing, the response falls back to the full-size image instead of leaving a gap. Deleting a photo deletes **both objects**, not just the big one.
...
* **One installation per salon, and a script that onboards it:** the business details (name, contact, branding and opening hours) live in the database and are edited from the admin panel, not in the deployment or in the app's code. `scripts/alta-cliente.sh` gets a freshly deployed installation ready — first admin account, business details and module profile — and `docs/alta-cliente.md` has the full order, including what cannot be automated.
* **Test suite (542 tests):** 452 unit tests covering the business logic without Spring context or database, plus 90 integration tests with **Testcontainers** (real PostgreSQL in Docker) covering authentication, ownership rules, statistics, payments, sales and commissions, role-based appointment closing, the business's switchable modules and the full Stripe webhook flow with real signature verification.
...
**542 tests** run in CI on every push (GitHub Actions).
...
| PagoServiceTest | 47 | PaymentIntents, webhooks, manual payment, refunds, polling, concurrency, who may request a receipt and in which payment states, and who may take cash: a barber needs the permission **and** the appointment to be on their own schedule, while an ADMIN never goes through permissions at all. **Modules** are tested here for the opposite reason: with charging off not even an ADMIN can take payment, each payment method checks its own module, and the webhook and the refund keep working, because switching a module off must never leave money in limbo at Stripe |
...
| GaleriaServiceTest | 35 | Gallery: uploads image and thumbnail to the public bucket, server-generated keys kept in separate folders per size, response falls back to the full-size image when there is no thumbnail, rejects non-images (thumbnail included), a new photo lands after the last one, deleting removes **both** objects, and the **owner of each photo**: an upload is stamped with the authenticated account, each hairdresser edits and deletes their own and not a colleague's, a photo with no owner belongs to the business and counts as someone else's, reordering is a separate permission because it moves everyone's grid, and an ADMIN never goes through the permission matrix. The **module**: switched off there is no gallery **even for an ADMIN**, and it is not listed without an account either - returning an empty list would say "there are no photos", which is a different thing |
...
| ModuloServiceTest | 23 | The business's modules, which are the opposite of a permission: they are **born switched on** (deploying them takes nothing away from anyone), switching the parent off switches the children off, switching every child off switches the parent off in effect (otherwise charging would stay "on" with no way to charge), a child switched on under a switched-off parent is reported as not effective so the screen cannot lie, the 409 names the parent when the parent is the one that is off, a row for a module removed from the code is ignored, and writing drops the cache. Plus the **start-up profiles**, which write every module's state at once to onboard a new salon: applying one twice leaves the same thing, they drop the cache, and neither "appointments only" nor "appointments and till" switches the card gateway on, because Stripe needs an account of each business's own |
| NegocioServiceTest | 11 | The business details, which are what it takes to install this for another salon: opening hours now come from the table and no longer from `application.properties` (while they lived there they could not differ per client without editing the deployment and restarting), they are read once (cache) and saving drops it, an opening time at or after closing time is rejected with a message instead of letting the table CHECK blow up as a 500, empty optional fields are stored as null, a weekday that does not exist is ignored rather than breaking start-up, and a missing row says the installation is incomplete instead of inventing a name and a timetable |
...
| PeluqueroCvServiceTest | 16 | The public CV: the listing comes from the repository that **filters inactive profiles and orders them** (not the plain `findByActivoTrue`, which guarantees no order), a profile with no photo does not invent a URL, an account with no profile has no CV, without `PERFIL_CV_EDITAR` it cannot be filled in, an ADMIN never goes through the matrix, a blank field **clears** instead of storing whitespace, only the Instagram handle is stored even when the full URL is pasted, a stylist cannot touch a colleague's photo even with the permission on, and replacing the photo **stores before deleting** the previous one. The **module**: switched off no profiles are published and no CV is edited, neither their own nor anyone else's |
| EspecialidadesTest | 8 | The only piece holding up storing specialties as comma-separated text: splitting and cleaning both ways, dropping duplicates case-insensitively, rejecting a tag **containing a comma** (it would split the list on read) and rejecting what does not fit the column even when every tag is valid on its own |
| PeluqueroServiceTest | 25 | CRUD and soft delete, account linking (rejects a `USER`, a deactivated account and one already linked to another profile), unlinking, and the applicable commission: the per-service exception beats the profile's rate. With the **commissions module off**, its endpoint answers 409, the profile comes back with no rate and no exceptions -the stored value is untouched- and the name can still be edited: what disappears is the money, not the profile |
| ProduccionServiceTest | 15 | Mapping of the summary, the breakdowns and the outstanding amount, account with no linked profile, inverted and over-two-year ranges, and rounding to two decimals what Postgres returns. Plus both modules: with **payments off** it counts what was done even if unpaid (otherwise all sales would be zero) and the "uncharged" figure disappears, which would otherwise count the same work twice; with **commissions off** the commission travels in neither the total, nor the lines, nor the staff comparison |
...
| RecordatorioCitaSchedulerTest | 6 | 24h reminder: sends once, skips cancelled/already-notified, injectable Clock. and with the module off it sends nothing **and marks no appointment as notified**, which is what would leave that whole window without a reminder when it is switched back on |
...
### Integration tests (90, Testcontainers)
...
* **GaleriaIntegrationTest** (4) — the gallery's asymmetry over HTTP: the listing is read **without a token**, while uploading, reordering and deleting return 403 with no token and as a client. Another one covers what a service unit test cannot see: a photo uploaded without a thumbnail returns the full-size image in its place, and an executable renamed to `.jpg` still ends in 400 after going through a real multipart request. The last two cross **two hairdressers and their photos**: each one manages their own and gets 403 on a colleague's, the listing flags which ones are theirs without publishing their id or email, reordering reaches any photo because it renumbers the whole grid, and ownerless photos (the business's) are only touched by an ADMIN.
* **PeluqueroCvIntegrationTest** (7) — the CV is the second resource served **without an account**, and the first one coming out of a table that also stores the linked account and what each person earns; that is why the JSON of `/api/peluqueros/publicos` is checked over HTTP to carry **no email, phone, `usuarioId`, commission or `activo`**, and that its rule really sits before `/api/peluqueros/**` in the filter chain. Also: a profile with no CV shows only the name and an empty list (not `null`, which would break the `*ngFor`), only active profiles appear and in the order the ADMIN set with the name as tie-breaker, reading your own profile needs no permission but writing it does, `PUT /{id}/cv` is ADMIN-only even on your own profile (that is what `/mio` is for), clearing a field means sending it blank, a specialty with a comma and a set that does not fit the column end in 400 instead of a Postgres 500, and the photo is set by the profile's owner or an ADMIN, never by a colleague.
...
* **ModuloIntegrationTest** (17) — modules end to end, which is where you see what a permission cannot do: with charging off **an ADMIN gets a 409** (not a 403: it is not "you may not", it is that nobody charges here), switching off just the card leaves in-salon payment standing, and switching it back on restores charging without a restart. Also: `/api/modulos/activos` is read **without a token** because the app has public screens, the catalogue and its writes are ADMIN-only, switching off all three payment methods switches off charging altogether, and -the trap this design had to avoid- **with payments off, sales count what was done** instead of dropping to zero, without touching the rate already frozen on closed appointments. Plus the four modules that **remove a whole part of the product**: switching off the gallery or the team CV takes their listing down **even without a token** (a 409 says "there is no gallery here", which is not the same as an empty list), picking who to book with still works because that does not come from the CV, and switching off sales answers 409 **to an ADMIN too**. Plus the **start-up profiles**: applying one switches off everything outside it at once, the till profile does not switch the card gateway on, only an ADMIN may apply one, and a profile that does not exist is a 400 and not a 500.
* **NegocioIntegrationTest** (7) — the business details end to end, which is where the two things that justify this table show up: the row **is seeded by the migration** (a freshly deployed installation already has a timetable and can take bookings without anyone filling anything in; without it the first `/api/citas` would be a 500), and **changing the timetable from the admin panel moves the slots offered when booking**, which used to mean editing the deployment and restarting. Also: the details are read **without a token** because both apps paint them before login, editing is ADMIN-only, a backwards timetable answers 400 rather than a 500 from the table CHECK, the name is required and the rest is not (a newly onboarded salon may not have a phone number yet), and what was saved shows up on the next read without a restart.
* **PermisoIntegrationTest** (11) — configurable permissions end to end, which is where the split between the two layers shows: `SecurityConfig` lets through whoever *could* hold the permission and the service decides whether they actually do. With the permission off a barber gets 403 taking cash while the ADMIN still can; switched on they take their own but **not a colleague's**; a customer gains nothing from it being on, because the role rule stops them first; switching it off takes it away again (the cache does not make it irrevocable); everyone reads their own and only the ADMIN sees the matrix, which offers no checkbox for ADMIN or customer; and writing an unknown key or a role that is not configurable returns 400 instead of leaving a row nobody would read. And the catalogue, which is not a permission: a barber with the whole matrix off reads `GET /api/servicios` with their token, because the app gives them a tab with it, and a service the ADMIN has retired no longer shows up.
...
| POST | `/api/galeria` | PELUQUERO, ADMIN | Upload a photo at the end of the grid (`multipart/form-data`: `imagen`, optional `miniatura`, optional `titulo`). It is stamped with the uploading account |
| PUT | `/api/galeria/{id}` | Owner, ADMIN | Change the title or the position |
| DELETE | `/api/galeria/{id}` | Owner, ADMIN | Delete the photo and both of its objects in the store |

> An ADMIN touches any photo. A PELUQUERO depends on their permissions: `GALERIA_SUBIR` to publish, `GALERIA_EDITAR_PROPIA` for the title and deletion of **their own**, `GALERIA_EDITAR_AJENA` for someone else's and `GALERIA_ORDENAR` to move the grid, which is separate because placing one photo renumbers everyone's. Ownerless photos belong to the business and count as someone else's.
...
| Method | Endpoint | Access | Description |
|--------|----------|--------|-------------|
| GET | `/api/peluqueros/publicos` | **Public** | The team with their CV, to pick before signing up. Active only and ordered; publishes nothing from the account and no commission |
...
| PUT | `/api/peluqueros/{id}` | ADMIN | Update a barber: `nombre`, `activo`, `comisionPorcentaje`, `orden`, `usuarioId` to link the account or `desvincularUsuario` |
| GET | `/api/peluqueros/mio` | PELUQUERO, ADMIN | Their own CV, resolved from the account (no id, like `/produccion/mia`) |
| PUT | `/api/peluqueros/mio` | PELUQUERO with `PERFIL_CV_EDITAR`, ADMIN | Replaces their whole CV: whatever is not sent is cleared |
| PUT | `/api/peluqueros/{id}/cv` | ADMIN | Any profile's CV. No permission opens this one to a stylist |
| POST | `/api/peluqueros/{id}/foto` | Profile owner with `PERFIL_CV_EDITAR`, ADMIN | CV photo (multipart `foto`, validated by magic bytes) |
| DELETE | `/api/peluqueros/{id}/foto` | Profile owner with `PERFIL_CV_EDITAR`, ADMIN | Remove the CV photo (idempotent) |
...
>
> Unless the business **does not charge through the app**: with the `PAGOS` module off nothing would ever reach `PAGADO`, so there it counts plain `COMPLETADA` and the response says so in `exigeCobro`. With `COMISIONES` off the commission travels as `null`, which is not zero: zero would mean "works at 0 %".
...
| Method | Endpoint | Access | Description |
|--------|----------|--------|-------------|
| GET | `/api/modulos/activos` | **Public** | Keys of what this business has switched on. No token, because the app has screens shown without an account and needs to know what to render before the login |
| GET | `/api/modulos` | ADMIN | Full catalogue with its state, for the configuration screen |
| PUT | `/api/modulos` | ADMIN | Switch on and off (`cambios[]` with `clave` and `activo`) |
| GET | `/api/modulos/perfiles` | ADMIN | The start-up profiles, with what each one switches on and off |
| POST | `/api/modulos/perfiles/{clave}` | ADMIN | Apply a profile: writes **every** module's state at once |

> **A module is not a permission, and confusing them breaks the design.** A permission answers "who may do X" and never reaches the ADMIN; a module answers "does this business do X at all", and switched off it **disappears for the administrator too**. That is why it answers **409** and not 403: it is not "you may not", it is that here it does not exist; and not 404, which would suggest the deployed backend is old. The order of the checks is always **module -> role -> permission**.
>
> As with permissions, the catalogue is defined by the `Modulo` enum and the table only stores state. The difference is the default: **they are all born switched on**, because the default must be the previous behaviour. Payment methods hang off `PAGOS`: switching the parent off switches them off, and switching them all off switches the parent off, so charging never stays "on" with no way to charge.
>
> The catalogue today is nine: `COMISIONES`, `PAGOS` with its three payment methods, and the four that remove a whole part of the product - `GALERIA`, `EQUIPO_CV`, `PRODUCCION` and `RECORDATORIOS_EMAIL`. The **assistant is not there, and should not be**: it is switched off through deployment configuration because it burns paid-API quota, so it is infrastructure cost rather than a business decision; the screen says so, so the question does not come back every time.
>
> Switching reminders off **marks no appointment as notified**: if it did, switching them back on would leave every appointment in that window without a reminder. The scheduler keeps running and simply sends nothing. It does not reach the other emails either: a booking or cancellation message confirms something that just happened, it is not a reminder.
>
> Switching a module off **deletes no data and rewrites no history**: stored rates stay, a closed appointment keeps its frozen commission, and a live `PaymentIntent` can still be charged or cancelled. What is switched off is what is offered and calculated from now on.
>
> **The start-up profiles** exist because "all switched on" is right for a business that was already running and exactly wrong for one that has just arrived: that one is shown the whole product at once and has nine things to switch off by hand. There are three - `SOLO_AGENDA`, `AGENDA_Y_CAJA` and `TODO` - and **a profile describes the complete state**, not an addition on top of whatever was there: everything outside it is switched off, so applying it twice leaves the same thing. No column records which one was applied, because that would be a value that starts lying the moment someone touches a checkbox. And neither `SOLO_AGENDA` nor `AGENDA_Y_CAJA` switches `PAGO_TARJETA` on: Stripe needs an account and keys of each business's own, so on a new client's first day switching it on would offer the end customer a gateway that returns an error.

### The business
| Method | Endpoint | Access | Description |
|--------|----------|--------|-------------|
| GET | `/api/negocio` | **Public** | Name, contact, branding and opening hours. No token, because both apps paint them before login |
| PUT | `/api/negocio` | ADMIN | Save the whole record |

> **This is what it takes to install this for another salon.** Opening hours lived in `application.properties` (`peluqueria.horario.*`) and the name, phone and address were written into the mobile app's code: changing them meant editing the deployment and restarting, or rebuilding and reinstalling the APK. Since `V17` it is a row in the `negocio` table and a form in the admin panel.
>
> **One single row, enforced with a CHECK** rather than trusting that nobody inserts another. This backend serves one salon: the day it serves several, this is the table that carries the `negocio_id` and the CHECK goes. Making the single row explicit is what makes that day noticeable.
>
> The **timetable here is the fixed weekly one** (what time it opens and which weekdays it never opens). Public holidays and one-off closures are still `dias_bloqueados`, which go by date and are a different question. What is **not** here because it belongs to the installation: the email sender (`MAIL_FROM`, imposed by the provider) and the Stripe keys.
>
> It is cached in memory for the same reason as modules and permissions - the timetable is asked on the path of every request that books or paints slots - and saving drops the cache, so changing it moves the offered slots without a restart.
...
3. **Set environment variables** (in your system or IDE):
    ...
    *(Opening hours are not a property: they live in the `negocio` table, seeded by the migration and edited from the admin panel. A fresh install starts at 09:00-20:00 with Sunday closed.)*
